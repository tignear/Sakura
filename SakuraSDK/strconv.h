...
static inline std::string wide_to_cp(std::wstring_view s, UINT codepage)
{
	int in_length = (int)s.length();
	int out_length = WideCharToMultiByte(codepage, 0, s.data(), in_length, 0, 0, 0, 0);
	std::string result(out_length, '\0');
	if (out_length) WideCharToMultiByte(codepage, 0, s.data(), in_length, &result[0], out_length, 0, 0);
	return result;
}
static inline std::wstring cp_to_wide(std::string_view s, UINT codepage)
{
	int in_length = (int)s.length();
	int out_length = MultiByteToWideChar(codepage, 0, s.data(), in_length, 0, 0);
	std::wstring result(out_length, L'\0');
	if (out_length) MultiByteToWideChar(codepage, 0, s.data(), in_length, &result[0], out_length);
	return result;
}
